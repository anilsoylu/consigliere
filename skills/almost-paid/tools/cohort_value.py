...
def emit(data):
    json.dump({"ok": True, "tool": TOOL, "data": data}, sys.stdout, ensure_ascii=False, indent=2, default=str)
    sys.stdout.write("\n")
    sys.exit(0)


def fail(message, code="error", exit_code=1):
    json.dump({"ok": False, "tool": TOOL, "error": {"code": code, "message": message}}, sys.stdout, ensure_ascii=False, indent=2, default=str)
    sys.stdout.write("\n")
    sys.exit(exit_code)
...
def load_users(path: str) -> list[dict]:
    ...
    if not isinstance(users, list):
        fail("Input must be a CSV, a JSON list of users, or {\"users\": [...]}", code="bad_input", exit_code=2)
    return users
...
def main() -> None:
    ...
    for key, label, temperature, default_rate, offer in SEGMENTS:
        ...
        if missing_arppu:
            fail(f"No --arppu for platform(s): {sorted(missing_arppu)} (pass --arppu {next(iter(missing_arppu))}=<price> or a bare --arppu for all)",
    ...
    if args.csv_dir:
        ...
        log(f"wrote {len(by_segment)} segment CSVs to {args.csv_dir}")

    if args.dashboard:
        template = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "templates", "dashboard.html")
        if not os.path.exists(template):
            fail(f"dashboard template not found at {template}", code="missing_template")
        # the template is prettier-formatted, so the placeholder may carry a space before null
        html = re.sub(r"/\*__DATA__\*/\s*null", lambda _: json.dumps(data, default=str), open(template, "r", encoding="utf-8").read(), count=1)
        with open(args.dashboard, "w", encoding="utf-8") as fh:
            fh.write(html)
        data["dashboard"] = args.dashboard
        log(f"wrote dashboard to {args.dashboard}")
