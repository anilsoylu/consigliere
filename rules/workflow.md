...
- A permission denial is information, not a wall. Read its stated reason before retrying. When it names a missing authorization, rewording the command cannot satisfy it — the same denial follows, and each attempt burns a turn. Ask once for the exact authorizing sentence, naming the tool, the target, and the irreversible effect ("say: run `asc metadata apply` against the live App Store listing"). A bare "yes, do it" will not clear an outward-facing gate, so don't treat one as if it did. A denial from the auto-mode classifier on push, merge or tag usually means the allow rule for that command is not in the user's settings; it can also be the target itself (someone else's repository, a force push, the wrong remote). Report the exact denied command once and stop. Only the user adds the rule (`node install.mjs --with-release-permissions` writes the release set); do not reword the command, do not hand it to another agent, and do not run the installer or edit settings.json to add the rule yourself.
